...
export const TaxRateStatusForm = ({ openStatusForm, setOpenStatusForm }) => {
    ...
    const { statusUploadLoading, changeTaxRateStatus, taxRateStatusSuccess } = useTaxRates()
    ...
    const onsubmit = (data) => {
        const { isActive, taxRateId, taxAccountId } = openStatusForm;
        data = { ...data, isActive, taxRateId, taxAccountId }
        changeTaxRateStatus(data)
    }

    useEffect(() => {
        if (!taxRateStatusSuccess) return
        setOpenStatusForm(prev => ({ openCom: false }))
