...
export const CreditAddBox = ({ setOpenRegisterDialog, invoiceNumber, totalAmount, title, invoiceId, setShowCreditDialog, resetBillingState, }) => {
    ...
    const { addCredit, isSuccess, isPending } = useCredit()
    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit} className='flex flex-col items-start justify-end mt-5 gap-3'>
                ...
                {creditAddData.creditAmount >= totalAmount || creditAddData.creditAmount === 0 ? "" : <p className='text-sm text-gray-600'>Customer Pay amount: {`(${totalAmount}) - (${creditAddData.creditAmount}) = `}  {totalAmount - creditAddData.creditAmount}</p>}

                {creditAddData.creditAmount >= totalAmount ? "" : <select onChange={(e) => setCreditAddData((prev) => ({ ...prev, paymentMethod: e.target.value }))} className=' h-10 w-full rounded-md bg-gray-100 p-2 tracking-wider'>
                    <option value={""}>Select Payment</option>
                    <option Value={"cash"}>Cash</option>
                    <option value={"internal-device"}>Card</option>
                </select>}
                <button className='w-full h-10 bg-primary mt-5 rounded-md text-white  disabled:opacity-40' disabled={!isFormValid || isPending}>{isPending ? "Credit Adding..." : "Credit Add"}</button>
            </form>
