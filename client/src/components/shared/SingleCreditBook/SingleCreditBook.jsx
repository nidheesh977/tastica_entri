...
export const SingleCreditBook = () => {
    ...
    const dataLoading = creditDataDisplay == null
    ...
    const filterDataDisplay = filterData.slice(0, loadMore).map((credit, index) => (
                ...
                <div >
                    <div className=' mt-4 w-[50%] mx-auto relative z-0'>
                        ...
                        {openFilterBox && <div className='absolute  flex flex-col gap-1 right-0 w-32 bg-slate-100 rounded p-2'>
                            <button onClick={() => handleFilterData("all")} className={`${filterValue === "all" ? "bg-blue-300 text-white" : "text-blue-500 "} rounded cursor-pointer`}>ALL</button>
                            <button onClick={() => handleFilterData("paid")} className={`${filterValue === "paid" ? "bg-blue-300 text-white" : "text-blue-500 "} rounded cursor-pointer`}>Paid</button>
                            <button onClick={() => handleFilterData("pending")} className={`${filterValue === "pending" ? "bg-blue-300 text-white" : "text-blue-500 "} rounded cursor-pointer`}>pending</button>
                        </div>}

                    </div>
                    {dataLoading ? "" : filterData.length === 0 ? <p className='text-center'>No Data found</p> : filterDataDisplay}
