...
import { MdPrint, MdShoppingCart } from "react-icons/md";
import { useNavigate } from "react-router-dom";
import { useQueryClient } from "@tanstack/react-query";
import { useEffect } from "react";
import { useSelector } from "react-redux";

export const PaymentSuccessSwipe = () => {
  ...
  return (
    <>
      <div className="flex flex-col justify-center items-center gap-4 mt-10 font-bold text-primary text-xl  w-96 h-96 mx-auto shadow-2xl rounded">
        ...
        <p>Payment success</p>
        <button
          className="bg-primary flex gap-2 items-center cursor-pointer hover:bg-opacity-90 text-white  px-2 text-base py-2 rounded"
          onClick={() => navigate(admin ? "/admin/cart" : "/staff")}
        >
          <MdShoppingCart /> Back to Home
        </button>
        <button
          className="bg-primary flex gap-2  items-center  cursor-pointer hover:bg-opacity-90 text-white px-11 text-base py-2 rounded"
          onClick={() => navigate(admin ? "/admin/print" : "/staff/print")}
        >
          <MdPrint /> Print
        </button>
      </div>
