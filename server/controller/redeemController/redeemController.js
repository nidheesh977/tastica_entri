...
export const addRedeemToInvoice = async (req, res) => {
    try {
        ...
        if (findInvoice.redeemAmount === redeemAmountAdd && findInvoice.redeemAmount !== 0) {
            ...
            const deductSubTotal = Number(parseFloat(findInvoice.subTotal - redeemAmountAdd).toFixed(2))
            const deductTotalAmount = Number(parseFloat(findInvoice.totalAmount - redeemAmountAdd).toFixed(2))
            ...
            const deductSubTotal = Number(parseFloat(findInvoice.subTotal + findInvoice.redeemAmount - redeemAmountAdd).toFixed(2));
            const deductTotalAmount = Number(parseFloat(findInvoice.totalAmount + findInvoice.redeemAmount - redeemAmountAdd).toFixed(2));
            const { loyalityPoint } = findCustomer;
            const updateInvoice = await invoiceModel.findByIdAndUpdate(findInvoice._id, {
                subTotal: deductSubTotal,
                totalAmount: deductTotalAmount,
