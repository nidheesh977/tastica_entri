...
export const softDeleteInvoice = async (req, res) => {
    try {
        ...
        const { error, value } = invoiceSoftDeleteValidation.validate(req.body)

        if (error) {
            return res.status(400).json({ success: false, message: error.details[0].message })
        }

        const { actions, archiveReason } = value;
        const { invoiceId } = req.params;
        ...
        if ("archive" === actions.toLowerCase()) {

            if (!archiveReason) {
                return res.status(400).json({ success: false, message: "Please enter any reason" })
            }

            await invoiceModel.findByIdAndUpdate(invoiceId, { invoiceStatus: "archived", archiveReason }, { new: true })

            res.status(200).json({ success: true, message: "Invoice delete successfully" })
        }
        else if ("restore" === actions.toLowerCase()) {
            await invoiceModel.findByIdAndUpdate(invoiceId, { invoiceStatus: "paid", archiveReason: null }, { new: true })
            res.status(200).json({ success: true, message: "Invoice restore successfully" })
        }
